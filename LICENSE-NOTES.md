...
| Bundled Node.js runtime (`standalone/node/`; staged by `script/windows/fetch-node-runtime.ps1` on Windows and `script/fetch-node-runtime.sh` elsewhere) | **MIT** for Node.js itself, plus the licenses of Node's own bundled dependencies, reproduced in the shipped `LICENSE` | Copyright Node.js contributors and the Node.js project |
